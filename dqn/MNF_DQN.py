...
class MNFBackbone(tf.Module):
    """
    Backbone of the Deep Q-Network (DQN) with Bayesian fully-connected layers that approximates the Q-function.
    The Bayesian fully-connected layers utilize multiplicative normalizing flows by Christos Louizos, Max Welling
    (Jun 2017).

    Takes 'num_states' inputs and outputs one Q-value for each action.
    """
    def __init__(self, num_states, hidden_units, num_actions, use_z=True, max_std=1.0):
        super(MNFBackbone, self).__init__()
        ...
        self.dense_mnf_out = DenseMNF(n_out=num_actions, use_z=use_z, max_std=max_std, n_flows_q=2, n_flows_r=2,
                                      flow_h_sizes=[32])
    ...
    def kl_div(self, same_noise=True):
        """
        Compute current KL-divergence of all Bayesian layers.
        Can be used as a regularization term during training.
        """
        kldiv = self.dense_mnf_out.kl_div(same_noise)
        return kldiv

    def reset_noise(self):
        """
        Re-sample noise/epsilon parameters of the MNF layers. Required for the case of having the same epsilon
        parameters across one batch.
        """
...
class MNFDQN(tf.Module):
    """
    Deep Q-Network utilizing Multiplicative Normalizing Flows for efficient sampling.
    """
    def __init__(self, num_states, num_actions, hidden_units, gamma, max_experiences, min_experiences, batch_size, lr,
        ...
        self.model = MNFBackbone(num_states, hidden_units, num_actions, use_z=True, max_std=0.5)
    ...
    def predict(self, inputs, same_noise=False, training=True):
        ...
        :param same_noise: uses the same epsilon parameter, if set to `True`.
    ...
    def get_action(self, states, same_noise=False, training=True):
        """
        Predict action with the MNF network. In each forward pass the weights are sampled from the weight posterior
        distribution. Hence, approximated Thomson sampling is performed. For uncertain weight posterior distributions
        the variance in the sampled values will be higher, leading inherently to more exploration
